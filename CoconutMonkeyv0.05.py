#       <CoconutMonkey, Recursive file consolidator, v0.05,Github,EffortlessTurtle>
...
def Mover():    #moves files
    ...
    while True: #Loop for multiple targets
        ...
        for root, dir, files in os.walk((os.path.normpath(src)), topdown=False):
            print('files for loop')
            i = 0
            for name in files:
                print('name in files loop')
                #print(type(name))  #Type: str
                print(str(i) + '<- mover i')
                i = i + 1
                #name2 = str(name)
                SourceFolder = os.path.join(src, name)      # NOTE: SOURCE PATH
                TargFolder = os.path.join(dest, name)
                try:
            
                    NameCheck = os.path.isdir(dest)
                    ...
                    if NameCheck == True:
                        ...
                        shutil.move(SourceFolder, TargFolder)
                        print(str(SourceFolder) + '  >  ' + TargFolder)    
                except FileNotFoundError as er1:
                    print('file not found, traceback -> ' + str(er1))
...
#Currently gives output, but never copies the items
