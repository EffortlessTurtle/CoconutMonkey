#       <CoconutMonkey, Recursive file consolidator, v0.07,Github,EffortlessTurtle>
...
# Iteration 7
# Currently doing a lot of work in Mover() to get it to read and move the files. Does move files now, but 
# it took the whole tree
# instead of just the files. Still haven't fixed the renaming sequence.
# Notes:
...
def POBox():    #Determines TARGET
    ...
    if choose.lower() in choices:
        if choose.lower() == choices[0]:
            ...
            #If user chooses n
            #print(str(dest) + '<- POBox dest')  # NOTE: Signpost
            return abspath(dest)  
...
def Mover():    #moves files
    ...
    while True: #Loop for multiple targets
        ...
        for root, dirs,files in os.walk((os.path.normpath(src)), topdown=False):
            ...
            for names in files:
                print(str(src) + ' <- src, Mover() PATH 3')
                print(str(dest) + ' <- dest, Mover() PATH 3')     
                print(str(i) + ' <- mover i')  #NOTE: signpost
                i = i + 1               
                SourceFolder = src
                TargFolder = dest
                print(str(SourceFolder) + ' <- Source Folder, Mover() PATH')
                print(str(TargFolder) + ' <- Targ Folder, Mover() PATH')
                try:
                    print(str(SourceFolder) + ' <- Source Folder, Mover() PATH2')
                    print(str(TargFolder) + ' <- Targ Folder, Mover() PATH2')                    
                    NameCheck = os.path.isdir(TargFolder)
                    if NameCheck == True:
                        for name in names:
                            print(str(name) + ' <- name, Mover()')
                            TargFolder = os.path.join(TargFolder, name)
                            print(str(os.path.splitdrive(TargFolder)) + ' <-splitdrive output, Mover()')
...
def test():     # NOTE: signpost
    ...
    while True:
       ...
       #     print(os.path.split(list_of_files[i]))
       #     i += 1
